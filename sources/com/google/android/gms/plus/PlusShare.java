...
public final class PlusShare
{
    public static class Builder
    {
        ...
        {
            zzv.zza(zzayV, "Must include the launching activity with PlusShare.Builder constructor before setting call-to-action");
            Bundle bundle;
            boolean flag;
            if(uri != null && !TextUtils.isEmpty(uri.toString()))
                ...
                flag = false;
            zzv.zzb(flag, "Must provide a call to action URL");
            bundle = new Bundle();
            ...
            {
                zzv.zza(PlusShare.zzdm(s1), "The specified deep-link ID was malformed.");
        ...
        public Intent getIntent()
        {
            ...
            if(!flag || !flag4)
                ...
                flag1 = false;
            zzv.zza(flag1, "Call-to-action buttons are only available for URLs.");
            if(!flag4 || mIntent.hasExtra("com.google.android.apps.plus.CONTENT_URL"))
                flag1 = true;
            else
                flag1 = false;
            zzv.zza(flag1, "The content URL is required for interactive posts.");
            flag1 = flag2;
            if(flag4)
            {
                flag1 = flag2;
            ...
            zzv.zza(flag1, "Must set content URL or content deep-link ID to use a call-to-action button.");
            if(mIntent.hasExtra("com.google.android.apps.plus.CONTENT_DEEP_LINK_ID"))
                zzv.zza(PlusShare.zzdm(mIntent.getStringExtra("com.google.android.apps.plus.CONTENT_DEEP_LINK_ID")), "The specified deep-link ID was malformed.");
            if(!flag && flag3)
            {
                mIntent.setAction("android.intent.action.SEND");
                if(zzayW != null && !zzayW.isEmpty())
                    mIntent.putExtra("android.intent.extra.STREAM", (Parcelable)zzayW.get(0));
                else
            ...
            if(flag && !flag3)
            {
                ...
                if(zzayW != null && !zzayW.isEmpty())
                    mIntent.putParcelableArrayListExtra("android.intent.extra.STREAM", zzayW);
                else
                    mIntent.removeExtra("android.intent.extra.STREAM");
            }
        ...
        {
            zzv.zzb(zzayV, "Must include the launching activity with PlusShare.Builder constructor before setting deep links");
            boolean flag;
            if(!TextUtils.isEmpty(s))
                flag = true;
            else
                flag = false;
            zzv.zzb(flag, "The deepLinkId parameter is required.");
